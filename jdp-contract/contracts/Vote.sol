...
contract Vote is ERC20 {
    ...
    function dropUnderSecondDecimal(uint256 value) view internal returns (uint256){
        uint8 decimal_ = decimals() - 2 ;
        require(decimal_ > 0 && decimal_ < 25);
        uint256 dropDecimal = 10 ** uint256(decimal_);
        return value.sub(value.mod(dropDecimal));
    }
    ...
    function distributeReward(uint index) internal {
        ...
        for (uint i = 0; i < eventList[index].candidate.length; i++) {
            eventList[index].finalReward[eventList[index].candidate[i]] = dropUnderSecondDecimal(candidatePot.mul(eventList[index].received_token[eventList[index].candidate[i]]).div(totalReceivedToken));
        ...
        for (uint i = 0; i < eventList[index].voter.length; i++) {
            eventList[index].finalReward[eventList[index].voter[i]] = dropUnderSecondDecimal(voterPot.mul(eventList[index].finalVoterToken[eventList[index].voter[i]]).div(totalVoterToken));
